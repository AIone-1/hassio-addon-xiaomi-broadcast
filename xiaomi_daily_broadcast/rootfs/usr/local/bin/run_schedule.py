...
<div class="page" id="page-hist">
  ...
  <!-- 🛠️ 修复异常：只有选中有异常的日期才显示 -->
  <div class="card" id="fixCard" style="display:none">
    <button class="danger" onclick="fixAnomaly()">🛠️ 修复异常传感器数据</button>
    <div class="fmt-hint" style="margin-top:4px">该天检测到传感器异常（如某设备一天耗电几十度，不正常）。点这个修复（用该设备前两天正常值的平均值替换异常值）。</div>
...
// 🔑 页面历史栈：tab 切换记录，‹ › 按钮回退/前进
var pageHistory=[], pageHistoryIdx=-1, _histRestoring=false;
function pushPageHistory(p){
  if(_histRestoring) return;
  // 去掉当前索引之后的历史（前进分支被新操作覆盖）
  pageHistory = pageHistory.slice(0, pageHistoryIdx+1);
  pageHistory.push(p);
...
function histPick(ds){
  histSel=ds;
  histRenderCal();
  // 🔑 只有选中异常日期才显示修复按钮，正常日期隐藏
  var fc=document.getElementById('fixCard');
  if(fc) fc.style.display=(histAnomalies[ds]?'block':'none');
...
function loadHist(){
  histLoadCal();
  // 🔑 打开历史页默认选中并显示当天日期（不用手动点）
  var today=new Date();
  var ds=histFmt(today.getFullYear(),today.getMonth())+'-'+pad(today.getDate());
  setTimeout(function(){ histPick(ds); }, 300);  // 等日历渲染完再选中当天
}
...
setActive('');
loadEngine();
// 🔑 页面历史初始化：从 URL hash 恢复，否则默认播报页
pageHistory=['main']; pageHistoryIdx=0;
(function(){
  try{
    var h=location.hash.replace('#','');
    if(h && ['main','sec','cfg','tpl','llm','hist'].indexOf(h)>=0){ showPage(h); }
  }catch(e){}
})();
