...
<h1 style=\"display:flex;align-items:center;gap:8px\">🎙️ 小爱每日播报
  ...
  <button class=\"ghost\" onclick=\"gotoAutomations()\" style=\"padding:6px 12px;font-size:12px\">⚙️ HA 自动化</button>
...
function gotoAutomations(){
  // ingress 下 pathname 形如 /api/hassio_ingress/<token>/，去掉前缀得到 HA 根路径
  var m = location.pathname.match(/^\\/api\\/hassio_ingress\\/[^/]+\\//);
  var haRoot = m ? location.pathname.slice(0, location.pathname.indexOf(m[0])) : '';
  if(!haRoot) haRoot = '/';
  location.href = haRoot + 'config/automations';
}
